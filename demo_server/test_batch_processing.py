        ...
        cursor.execute(
            """INSERT INTO batch_jobs 
               (user_id, status, total_files, processed, failed, 
                ...
                tool_name, metadata, created_at, read) 
               VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) 
               RETURNING id""",
            (user_id, "processing", total_files, 0, 0, 0, 0, 0, 
             tool_name, Json({'batch_id': batch_id}), datetime.now(), False)
        )
...
def verify_results_in_database():
    ...
    try:
        ...
        cursor.execute("""
            SELECT id, status, total_files, processed, failed, created_at, metadata->>'batch_id' as batch_id 
            FROM batch_jobs 
            ORDER BY created_at DESC 
            LIMIT 5
        """)
